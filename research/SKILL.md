---
name: research
description: deep-research 内部可调用的调研模板包/取证层。用于按 academic / competitive / discovery / comprehensive 四类模板检索、筛证据、整理来源和产出 route-specific 结果包；不再作为默认用户可见总入口，除非用户显式点名。
status: draft
---
...
这是 `deep-research` 下面的内部调研模板包/取证层。它先判断问题属于哪一类证据任务，再把问题送进对应模板包，而不是用一个万能 prompt 糊所有问题。

默认用户可见总入口是 `deep-research`。`research` 只在上游已经确认要做模板化取证、或用户显式点名 `research` 时直接使用。
...
优先由 `deep-research` 内部调用。直接使用 `research` 只适用于用户显式点名，或上游流程已经完成 Gate / Clarify / Brainstorm / Grillme，明确需要模板化取证。
...
- 用户只是问名词解释、概念、定义
- 用户只是要写文案、起标题、改措辞
- 用户只是泛泛说“帮我调研”“研究一下”“做深度研究”，但还没有经过 `deep-research` 的入口判断
- 用户需求还需要 brainstorm 或 grillme
- 用户需要完整 PRD、研究设计或 deep-research 级别交付
- 用户的问题主要是执行型代码任务，而不是信息收集和判断

## 进入前先判断

进入 `research` 前必须先做一次轻判断；如果这是用户的第一句调研请求，默认应先回到 `deep-research`：

| 情况 | 处理 |
|---|---|
| 上游已明确要模板化取证，且需要外部证据 | 直接进入 `research` |
